...
class TaskExecutorNode(Node):
    ...
    def _task_assignment_callback(self, msg: Any) -> None:
        ...
        goal_msg.pose.pose = msg.target_pose

        self._nav_clients[robot_id].wait_for_server(timeout_sec=5.0)
        ...
        send_goal_future.add_done_callback(self._make_goal_response_callback(robot_id))

    def _make_goal_response_callback(self, robot_id: str):
        def _callback(future):
            ...
            result_future.add_done_callback(self._make_result_callback(robot_id))

        return _callback

    def _make_result_callback(self, robot_id: str):
        def _callback(future):
            ...
            if result.status == 4:  # SUCCEEDED
                ...
                self._task_min_distance.pop(robot_id, None)
                self.get_logger().info(
                    f'{robot_id} reached task {task.task_id if task else "?"} via Nav2'
                )
            else:
